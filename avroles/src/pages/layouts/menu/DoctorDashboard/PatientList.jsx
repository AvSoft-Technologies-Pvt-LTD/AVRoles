...
import { FaEye, FaEyeSlash, FaHeartbeat, FaNotesMedical, FaFlask, FaPills, FaSearch } from "react-icons/fa";
...
function getCurrentDate() { const d = new Date();return d.toISOString().slice(0, 10);}
...
const defaultIPD = Object.fromEntries(IPD.map(f => [f[0], f[0] === "admissionDate" ? getDate() : f[0] === "admissionTime" ? to24Hour(getTime()) : ""]));
...
const navigate = useNavigate();
  ...
  const [formData, setFormData] = useState({
    ...
    gender: "", dob: "", bloodGroup: "", addressPerm: "", addressTemp: "",
    password: "", confirmPassword: ""
  });
  const [appointment, setAppointment] = useState({
    date: getCurrentDate(), time: getCurrentTime(), reason: "", diagnosis: ""
  });
  ...
  const [formTouched, setFormTouched] = useState({}); // Track touched fields
...
const paginatedPatients = patients.slice(
  ...
  const F = (k, v) => setIpdState(s => ({ ...s, formData: { ...s.formData, [k]: v } }));
  const I = (k, v) => setIpdState(s => ({ ...s, ipdData: { ...s.ipdData, [k]: v } }));
  const validateIPD = (fields, data) => { let errs = {}; fields.forEach(f => { if (f[3] && !data[f[0]]) errs[f[0]] = `${f[1]} is required`; }); setIpdErrors(errs); return Object.keys(errs).length === 0; };
    ...
    if (activeTab === "OPD") {
      (async () => {
        try {
          ...
          setPatients(res.data.filter(p => (p.type || "opd").toLowerCase() === "opd").reverse());
        } catch (e) { }
      })();
    }
  }, [activeTab]);
useEffect(() => { const highlightIds = JSON.parse(localStorage.getItem("highlightOPDIds") || "[]"); const clicked = JSON.parse(localStorage.getItem("clickedOPDIds") || "[]"); setHighlightId(highlightIds); setClickedIds(clicked); setTimeout(() => { highlightIds.forEach(id => { if (!clicked.includes(id)) { const el = rowRefs.current[id]; if (el) el.scrollIntoView({ behavior: "smooth", block: "center" }); } }); }, 500); }, []);
    ...
    if (activeTab === "IPD") {
      (async () => {
        ...
        try {
          ...
          S("ipdPatients", d.reverse());
          S("filteredIPD", d.reverse());
  ...
  useEffect(() => {
    if (activeTab === "IPD") {
      ...
      ipdSearchTimeout.current = setTimeout(() => {
        ...
        S("filteredIPD", ipdState.ipdPatients.filter(p => (p.id || "").toString().toLowerCase().includes(val) || (p.name || "").toLowerCase().includes(val)));
        S("searching", false);
      }, 400);
  ...
  useEffect(() => { if (modalOpen || appointmentModal || selectedPatient || searchModal) { document.body.style.overflow = "hidden"; } else { document.body.style.overflow = ""; } return () => { document.body.style.overflow = ""; }; }, [modalOpen, appointmentModal, selectedPatient, searchModal]);
 ...
 useEffect(() => { localStorage.setItem("opdPatients", JSON.stringify(patients)); }, [patients]);
...
useEffect(() => { const highlightIds = JSON.parse(localStorage.getItem("highlightOPDIds") || "[]"); const clicked = JSON.parse(localStorage.getItem("clickedOPDIds") || "[]"); setHighlightId(highlightIds); setClickedIds(clicked); setTimeout(() => { highlightIds.forEach(id => { if (!clicked.includes(id)) { const el = rowRefs.current[id]; if (el) el.scrollIntoView({ behavior: "smooth", block: "center" }); } }); }, 500); }, []);
useEffect(() => { (async () => { try { const res = await axios.get("https://684be316ed2578be881cdb55.mockapi.io/addpatient"); const filteredPatients = res.data.filter(p => p.doctorName === DEFAULT_DOCTOR_NAME).map(p => ({ id: p.id, name: [p.firstName, p.middleName, p.lastName].filter(Boolean).join(" "), diagnosis: p.diagnosis, reason: p.reason, datetime: `${p.appointmentDate} ${p.appointmentTime}`, bloodGroup: p.bloodGroup || "", family: p.family || [], vitals: p.vitals || {}, height: p.height, weight: p.weight, email: p.email, phone: p.phone })).reverse(); localStorage.setItem("filteredPatients", JSON.stringify(filteredPatients)); setPatients(filteredPatients); } catch (e) { console.error("Error fetching appointments:", e); } })(); }, []);


const [activeDropdown, setActiveDropdown] = useState(null);

const addForm = (type) => { const forms = JSON.parse(localStorage.getItem("medicalForms") || "[]"); if (!forms.find((f) => f.type === type)) { forms.push({ id: Date.now(), type }); localStorage.setItem("medicalForms", JSON.stringify(forms)); } setShowForm(true); setActiveDropdown(null); };
 if (showForm) return <FormPage />;
const handleCloseAppointmentModal = () => { setAppointmentModal(false); setModalOpen(false); setSelectedPatient(null); setSelectedSearchPatient(null); setSearchModal(false); };
const handleOpenAddPatient = () => { if (!appointmentModal) setModalOpen(true); };


  const handleSearch = async () => { if (!searchTerm.trim()) return; try { const response = await axios.get("https://684be316ed2578be881cdb55.mockapi.io/addpatient"); const results = response.data.filter(p => (p.name || "").toLowerCase().includes(searchTerm.toLowerCase())); setSearchResults(results); } catch { setSearchResults([]); } };

 const handleCreateAppointmentFromSearch = async () => { if (!selectedSearchPatient || !validateAppointment()) return; try { const payload = { name: selectedSearchPatient.name, email: selectedSearchPatient.email, phone: selectedSearchPatient.phone, aadhaarId: selectedSearchPatient.aadhaarId, occupation: selectedSearchPatient.occupation, appointmentDate: appointment.date, appointmentTime: appointment.time, diagnosis: appointment.diagnosis, reason: appointment.reason, doctorName: DEFAULT_DOCTOR_NAME }; await axios.post('https://684be316ed2578be881cdb55.mockapi.io/addpatient', payload); alert("Appointment scheduled successfully!"); setAppointmentModal(false); setSearchModal(false); setSelectedSearchPatient(null); setAppointment({ date: getCurrentDate(), time: getCurrentTime(), reason: "", diagnosis: "" }); } catch (error) { alert("Failed to schedule appointment."); } };
const handleAddRecord = (patient) => {
    navigate("/doctordashboard/form", { state: { patient } }); // Navigate to FormPage with patient data
  };
const SuggestInput = ({ val, setVal, list, setList, data, placeholder }) => (<div className="floating-input relative" data-placeholder={placeholder}><input className="input-field peer" value={val} placeholder=" " onChange={e => { const v = e.target.value; setVal(v); setList(data.filter(o => o.toLowerCase().includes(v.toLowerCase()))); }} autoComplete="off" />{list.length > 0 && <ul className="absolute z-10 bg-white border w-full rounded shadow mt-1 max-h-40 overflow-y-auto">{list.map((item, i) => <li key={i} onClick={() => { setVal(item); setList([]); }} className="px-3 py-2 hover:bg-gray-100 cursor-pointer">{item}</li>)}</ul>}</div>);
...
const ipdTotalPages = Math.ceil(ipdState.filteredIPD.filter(p => (p.type || "").toLowerCase() === "ipd").length / ipdPageSize);
...
useEffect(() => {
  ...
  const fetchPatient = async () => {
    try {
      ...
      const found = data.find(p => (p.id || "").toString() === ipdState.aadhaarId.trim());
      if (found) {
        const name = `${found.firstName || ""} ${found.middleName || ""} ${found.lastName || ""}`.replace(/\s+/g, " ").trim();
        S("formData", { ...defaultForm, ...found, name, addressPerm: found.permanentAddress || "", addressTemp: found.temporaryAddress || "", email: found.email || "", dob: found.dob || "", occupation: found.occupation || "", bloodGroup: found.bloodGroup || "", password: found.password || "", confirmPassword: found.confirmPassword || "" });
  ...
  const savePatient = async () => {
    ...
    try {
      const patientData = {
        ...
        ...ipdState.ipdData,
        type: "ipd",
        name: `${ipdState.formData.firstName} ${ipdState.formData.middleName || ""} ${ipdState.formData.lastName}`.trim(),
      ...
      const url = ipdState.editModalOpen && ipdState.formData.id ? `${API.FORM}/${ipdState.formData.id}` : API.FORM;
      const method = ipdState.editModalOpen && ipdState.formData.id ? "PUT" : "POST";
      ...
      if (response.ok) {
        toast.success(ipdState.editModalOpen ? "Patient updated successfully!" : "Patient added successfully!");
        resetForm();
        handleCloseIpdModal();
        const updatedData = await (await fetch(API.FORM)).json();
        let newPatient = null;
        if (!ipdState.editModalOpen) {
          newPatient = updatedData.find(p => p.createdAt === patientData.createdAt);
          const newId = newPatient ? newPatient.id : updatedData[updatedData.length - 1]?.id;
          S("ipdPatients", [newPatient || patientData, ...updatedData.filter(p => (newPatient ? p.id !== newPatient.id : true))]);
          S("filteredIPD", [newPatient || patientData, ...updatedData.filter(p => (newPatient ? p.id !== newPatient.id : true))]);
  ...
  const resetForm = () => { S("formData", defaultForm); S("ipdData", defaultIPD); S("occList", []); S("aadhaarId", ""); S("readonlyFields", false); setIpdErrors({}); };
  const viewIpdPatientDetails = async a => {
    S("selectedPatient", a); S("personalDetails", null); S("family", []); S("vitals", null); setRecentPatientId(null);
    try {
      ...
      S("personalDetails", pd.find(x => (x.email || "").trim().toLowerCase() === email));
      S("family", fd.filter(f => (f.email || "").trim().toLowerCase() === email));
      S("vitals", vs.find(x => (x.email || "").trim().toLowerCase() === email) || null);
    } catch { }
  };
  const handleEdit = () => {
    S("editModalOpen", true); S("modalOpen", true); S("showIpdForm", true);
    S("formData", { ...defaultForm, ...ipdState.selectedPatient, id: ipdState.selectedPatient.id, addressPerm: ipdState.selectedPatient.permanentAddress || "", addressTemp: ipdState.selectedPatient.temporaryAddress || "", email: ipdState.selectedPatient.email || "", dob: ipdState.selectedPatient.dob || "", occupation: ipdState.selectedPatient.occupation || "", bloodGroup: ipdState.selectedPatient.bloodGroup || "", password: ipdState.selectedPatient.password || "", confirmPassword: ipdState.selectedPatient.confirmPassword || "" });
    S("ipdData", { ...defaultIPD, ...ipdState.selectedPatient });
    S("photoUrl", ipdState.selectedPatient.photoUrl || ""); S("selectedPatient", null);
  };
  const handleCloseIpdModal = () => { S("modalOpen", false); S("showIpdForm", false); S("editModalOpen", false); S("selectedPatient", null); resetForm(); };
  const handlePatientFormSubmit = e => { e.preventDefault(); if (validateIPD(FIELDS, ipdState.formData)) S("showIpdForm", true); };
  ...
  const renderFields = (fields, values, onChange, errors = {}, occList, setOccList) =>
    fields.map(f => (
     ...
     const validateAppointment = () => { const errors = {}; if (!appointment.date) errors.date = "Date is required"; if (!appointment.time) errors.time = "Time is required"; if (!appointment.diagnosis) errors.diagnosis = "Diagnosis is required"; if (!appointment.reason) errors.reason = "Reason is required"; setAppointmentErrors(errors); return Object.keys(errors).length === 0; };
const handleScheduleAppointment = async () => { if (!validateAppointment()) return; try { const payload = { firstName: formData.firstName, middleName: formData.middleName, lastName: formData.lastName, name: [formData.firstName, formData.middleName, formData.lastName].filter(Boolean).join(" "), email: formData.email, phone: formData.phone, permanentAddress: formData.addressPerm, temporaryAddress: formData.addressTemp, dob: formData.dob, occupation: formData.occupation, aadhaarId, appointmentDate: appointment.date, appointmentTime: appointment.time, diagnosis: appointment.diagnosis, reason: appointment.reason, doctorName: DEFAULT_DOCTOR_NAME, type: "OPD" }; await axios.post('https://684be316ed2578be881cdb55.mockapi.io/addpatient', payload); alert("Appointment scheduled successfully!"); setAppointmentModal(false); setModalOpen(false); setSelectedPatient(null); } catch (error) { console.error('Error scheduling appointment:', error); alert("Failed to schedule appointment."); } };
  // --- Add Patient Validation & Handlers ---
  const validateForm = (showAll = false) => {
    const errors = {};
    if ((showAll || formTouched.aadhaarId) && (!aadhaarId || aadhaarId.length !== 12 || !/^[0-9]+$/.test(aadhaarId))) errors.aadhaarId = "Aadhaar must be 12 digits";
    if ((showAll || formTouched.firstName) && (!formData.firstName || !/^[A-Za-z\s]+$/.test(formData.firstName))) errors.firstName = "First name is required and only letters";
    if ((showAll || formTouched.middleName) && formData.middleName && !/^[A-Za-z\s]+$/.test(formData.middleName)) errors.middleName = "Middle name must be only letters";
    if ((showAll || formTouched.lastName) && (!formData.lastName || !/^[A-Za-z\s]+$/.test(formData.lastName))) errors.lastName = "Last name is required and only letters";
    if ((showAll || formTouched.phone) && (!formData.phone || !/^\d{10}$/.test(formData.phone))) errors.phone = "Phone must be 10 digits";
    if ((showAll || formTouched.email) && (!formData.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email))) errors.email = "Valid email is required";
    if ((showAll || formTouched.gender) && (!formData.gender || formData.gender === "Select Gender")) errors.gender = "Gender is required";
    if ((showAll || formTouched.dob) && !formData.dob) errors.dob = "Date of birth is required";
    if ((showAll || formTouched.addressPerm) && !formData.addressPerm) errors.addressPerm = "Permanent address is required";
    if ((showAll || formTouched.addressTemp) && !formData.addressTemp) errors.addressTemp = "Temporary address is required";
    if ((showAll || formTouched.password) && (!formData.password || !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$/.test(formData.password))) errors.password = "Password must be at least 8 characters, include uppercase, lowercase, number, and special character";
    if ((showAll || formTouched.confirmPassword) && (formData.password !== formData.confirmPassword)) errors.confirmPassword = "Passwords do not match";
...
useEffect(() => { (async () => { try { const res = await axios.get("https://684be316ed2578be881cdb55.mockapi.io/addpatient"); const filteredPatients = res.data.filter(p => p.doctorName === DEFAULT_DOCTOR_NAME).map(p => ({ id: p.id, name: [p.firstName, p.middleName, p.lastName].filter(Boolean).join(" "), diagnosis: p.diagnosis, reason: p.reason, datetime: `${p.appointmentDate} ${p.appointmentTime}`, bloodGroup: p.bloodGroup || "", family: p.family || [], vitals: p.vitals || {}, height: p.height, weight: p.weight, email: p.email, phone: p.phone })).reverse(); localStorage.setItem("filteredPatients", JSON.stringify(filteredPatients)); setPatients(filteredPatients); } catch (e) { console.error("Error fetching appointments:", e); } })(); }, []);
  ...
  const handleCreateAppointment = async (formData, aadhaarId, occ) => {
    ...
    try {
      ...
      await axios.post('https://6801242781c7e9fbcc41aacf.mockapi.io/api/AV1/users', payload);
      toast.dismiss();
      toast.success("Patient added successfully!", { position: "top-right" });
      setAppointment({ date: getDate(), time: getTime(), reason: "", diagnosis: "" });
      setAppointmentModal(true);
      setModalOpen(false);
    } catch (error) {
      console.error('Error creating appointment:', error);
      toast.dismiss();
      toast.error("Failed to create appointment. Please try again.", { position: "top-right" });
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-6 w-full">
        <div className="flex gap-4">{["OPD", "IPD"].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-2 rounded-full font-semibold transition-all duration-300 ease-in-out border ${activeTab === tab ? "bg-[#0E1630] text-white border-[#0E1630] shadow-lg" : "bg-transparent text-[#01D48C] border-[#01D48C] hover:bg-[#01D48C] hover:text-white"}`}>{tab}</button>
        ))}</div>
        {activeTab === "OPD" && (
          <div className="flex gap-2 items-center">
            <input type="text" placeholder="Search patient by name" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-8 pr-5 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-xs" style={{ width: "300px" }} />
           <div className="flex gap-2 items-center"><button onClick={handleOpenAddPatient} className="btn btn-primary whitespace-nowrap px-4 py-2 text-xs flex items-center gap-2" disabled={appointmentModal}><Plus className="w-4 h-4" /> Add Patient</button></div>
          </div>
        )}
        {activeTab === "IPD" && (
          <div className="flex flex-row items-center gap-2">
            <div className="relative w-full md:w-64">
              ...
              <input type="text" placeholder="Search by Patient ID or Name..." value={ipdState.search} onChange={e => S("search", e.target.value)} className="pl-8 pr-5 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent w-full text-xs" />
              {ipdState.searching && (<Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 animate-spin w-4 h-4" />)}
            </div>
      ...
      {/* OPD Table/Modals */}
    {activeTab === "OPD" && (<div>{showForm ? (
          <FormPage />
        ) : (<table className="table-container"><thead><tr className="table-head bg-gray-100">{["ID", "Name", "Diagnosis", "Date & Time", "Actions"].map((h, i) => (<th key={i} className="py-2 border-b">{h}</th>))}</tr></thead><tbody className="table-body overflow-visible">
  {paginatedPatients.filter((p) =>
    ...
    paginatedPatients
      ...
      .map((p, i) => (
          ...
          onClick={() => {
            ...
            if (highlightId.includes(strId)) {
              setClickedIds((prev) => {
                ...
                localStorage.setItem("clickedOPDIds", JSON.stringify(updated));
                return updated;
              });
            }
          }}
       ...
       <div className="flex justify-end items-center mt-4"><div className="flex items-center gap-2"><button disabled={state.currentPage === 1} onClick={() => setState(prev => ({ ...prev, currentPage: prev.currentPage - 1 }))} className={`edit-btn ${state.currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''}`}>Previous</button><span>Page {state.currentPage} of {totalPages || 1}</span><button disabled={state.currentPage === totalPages || totalPages === 0} onClick={() => setState(prev => ({ ...prev, currentPage: prev.currentPage + 1 }))} className={`edit-btn ${state.currentPage === totalPages || totalPages === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}>Next</button></div></div></div>)}
      {/* IPD Table/Modals */}
      {activeTab === "IPD" && (
            ...
            <form className="space-y-6" onSubmit={handlePatientFormSubmit}>
              <div className="flex flex-col md:flex-row gap-4">
                <div className="floating-input relative flex-1" data-placeholder="Enter PatientId Number">
                  <input className={`input-field peer ${ipdErrors.aadhaarId ? "border-red-500" : ""}`} value={ipdState.aadhaarId} maxLength={12} onChange={e => S("aadhaarId", e.target.value.replace(/\D/g, ""))} placeholder=" " autoComplete="off" />
                  {ipdErrors.aadhaarId && <span className="text-red-500 text-xs">{ipdErrors.aadhaarId}</span>}
                </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-[rgba(14,22,48,0.18)] backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl border-2 border-[var(--primary-color)] p-8 w-full max-w-xl animate-slideUp">
            ...
            <form className="space-y-6" onSubmit={handleIPDFormSubmit}>
              ...
              <div className="flex flex-col md:flex-row justify-end gap-4 pt-2">
                <button type="button" className="bg-gray-400 text-white px-6 py-2 rounded shadow hover:bg-gray-500 animated-cancel-btn" onClick={handleCloseIpdModal}>{ipdState.editModalOpen ? "Cancel" : "Back"}</button>
                <button type="submit" disabled={ipdState.saving} className="btn btn-primary animate-pulse-save flex items-center gap-2 disabled:opacity-50 px-6 py-2 shadow-md hover:scale-105 transition-all duration-300">
                  {ipdState.saving && <Loader2 className="w-4 h-4 animate-spin" />}
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-[rgba(14,22,48,0.18)] backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl border-2 border-[var(--primary-color)] p-6 w-full max-w-xl relative overflow-y-auto max-h-[90vh] animate-slideUp">
            ...
            <div className="border p-2 rounded mb-2">
              ...
              <div className="text-xs">{ipdState.selectedPatient.name} | Admission: {ipdState.selectedPatient.admissionDate} {ipdState.selectedPatient.admissionTime} | Status: {ipdState.selectedPatient.status}</div>
              <div className="text-xs">Reason: {ipdState.selectedPatient.reasonForAdmission}</div>
            </div>
              ...
              <div className="border p-2 rounded">
                ...
                <div className="grid grid-cols-3 gap-2 text-xs">
                  {["height", "weight", "bloodGroup"].map((k, i) => (
                    <div key={i}>{k.charAt(0).toUpperCase() + k.slice(1)}: {ipdState.personalDetails?.[k] || ipdState.selectedPatient[k] || (k === "bloodGroup" ? "Not recorded" : "—")}{k === "height" && " cm"}{k === "weight" && " kg"}</div>
                  ))}
                </div>
              </div>
              <div className="border p-2 rounded">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {[
                    ...
                    <div key={i}>{label}: {ipdState.selectedPatient[key] || "—"}</div>
                  ))}
                </div>
              </div>
              <div className="border p-2 rounded">
                ...
                <div className="text-xs">
                  <div>Permanent Address: {ipdState.selectedPatient.permanentAddress || "—"}</div>
                  <div>Temporary Address: {ipdState.selectedPatient.temporaryAddress || "—"}</div>
                  <div>Email: {ipdState.selectedPatient.email || "—"}</div>
                  <div>DOB: {ipdState.selectedPatient.dob || "—"}</div>
                  <div>Occupation: {ipdState.selectedPatient.occupation || "—"}</div>
                  <div>Blood Group: {ipdState.selectedPatient.bloodGroup || "—"}</div>
                  <div>Password: {ipdState.selectedPatient.password ? "******" : "—"}</div>
                  <div>Confirm Password: {ipdState.selectedPatient.confirmPassword ? "******" : "—"}</div>
                </div>
              </div>
              <div className="border p-2 rounded">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {[
                    ...
                    <div key={i}>{label}: {ipdState.vitals?.[key] || ipdState.selectedPatient.vitals?.[key] || "—"}</div>
                  ))}
                </div>
              </div>
