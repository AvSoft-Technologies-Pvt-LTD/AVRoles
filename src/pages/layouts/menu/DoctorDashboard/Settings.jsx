...
const formFields = {
  personal: [
    ...
    { id: "registrationNumber", label: "Registration Number", type: "tel" },
    { id: "practiceType", label: "Practice Type", type: "text" },
    { id: "specializationType", label: "Specialization Type", type: "text" },
    { id: "qualification", label: "Qualification", type: "text" },
    { id: "doctorType", label: "Doctor Type", type: "select", options: ["Freelancer", "Hospital Associated"] },
...
const Settings = () => {
  ...
  const apiUrl = "https://68147b27225ff1af1628f902.mockapi.io/doctorsettings";
  ...
  useEffect(() => {
    (async () => {
      ...
      setError(null);
      try {
        const { data: user } = await axios.get(apiUrl);
  ...
  const handleInputChange = e => {
    ...
    setFormData(prev => {
      ...
      if (name === "doctorType" && value === "Freelancer") updated.hospitalName = "";
      return updated;
    });
    setHasUnsavedChanges(true);
    saveSuccess && setSaveSuccess(false);
  };
  ...
  setIsSaving(true);
  try {
    const { data } = await axios.put(`${apiUrl}/${formData.id}`, formData);
  ...
  const renderField = ({ id, label, type, readOnly, options, toggleVisibility }) => {
    if (id === "hospitalName" && formData.doctorType !== "Hospital Associated") return null;
    const value = formData[id] || "";
    const baseClasses = `w-full px-4 py-3 rounded-xl transition-all duration-300 focus:outline-none ${
      readOnly
        ? "bg-gray-50 text-gray-500 border border-gray-200 cursor-not-allowed"
        : "bg-white border border-gray-300 focus:ring-2 focus:ring-[#F4C430] focus:border-purple-300"
    }`;
    let field;
    if (type === "textarea") field = (
      <textarea name={id} value={value} onChange={handleInputChange} className={`${baseClasses} resize-none`} rows={3} readOnly={!isEditMode} />
    );
    ...
    else if (type === "password") field = isEditMode ? (
      ...
      <input type={type} name={id} value={value} onChange={handleInputChange} className={baseClasses} readOnly={readOnly || !isEditMode} />
    );
    ...
    return (
      <div key={id} className="md:col-span-1">
        ...
        {isEditMode ? field : <div className="py-3 px-4 bg-gray-50 rounded-xl text-gray-700 border border-gray-200">{value || <span className="text-gray-400 italic">Not provided</span>}</div>}
      </div>
    );
  };

  const getTabIcon = tab => {
    if (tab === "personal") return <User  size={16} />;
    if (tab === "password") return <Lock size={16} />;
    return null;
  };
    ...
    <div className="mx-auto relative pb-20">
        ...
        <div className="relative group">
          <div className="w-32 h-32 rounded-full overflow-hidden ring-4 ring-white shadow-lg transition-all duration-300 group-hover:shadow-xl">
            {profileImage
              ...
              : <div className="w-full h-full flex items-center justify-center bg-purple-50 text-purple-300"><Camera size={32} /></div>
            }
          </div>
          {isEditMode && (
            <div onClick={() => fileInputRef.current?.click()} className="absolute bottom-0 right-0 w-10 h-10 bg-[#F4C430] rounded-full flex items-center justify-center cursor-pointer hover:bg-[#F4C430]/90 hover:shadow-lg transition-all duration-300 transform hover:scale-110">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-200 -mt-16 pt-20 pb-8 px-6 md:px-8">
        <div className="text-center mb-8">
          ...
          <h3 className="text-2xl font-bold text-gray-800 mb-1">{formData.firstName} {formData.lastName}</h3>
        ...
        <div className="mb-6 bg-gray-50 rounded-2xl p-2 border border-gray-200 flex flex-wrap gap-2">
          {["personal", ...(isEditMode ? ["password"] : [])].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`flex items-center gap-2 px-5 py-3 rounded-xl capitalize transition-all duration-300 ${activeTab === tab ? "bg-[#F4C430] text-white" : "text-gray-600 hover:bg-gray-100"}`}>
              <span>{getTabIcon(tab)}</span><span className="font-medium">{tab}</span>
            </button>
          ))}
        </div>
          ...
          {["personal", ...(isEditMode ? ["password"] : [])].map(tab => (
            <div key={tab} className={`transition-all duration-500 ${activeTab === tab ? "opacity-100 transform translate-y-0" : "hidden opacity-0 transform translate-y-8"}`}>
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{formFields[tab].map(renderField)}</div>
              </div>
            </div>
          ))}
          {isEditMode && (
            <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg p-4 flex justify-end gap-4 z-10">
              ...
              <button type="submit" disabled={!hasUnsavedChanges || isSaving} className={`px-6 py-2.5 rounded-xl flex items-center gap-2 transition-all duration-300 ${!hasUnsavedChanges || isSaving ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-[#F4C430] text-white shadow-md hover:bg-[#F4C430] hover:shadow-lg hover:-translate-y-1"}`}>
