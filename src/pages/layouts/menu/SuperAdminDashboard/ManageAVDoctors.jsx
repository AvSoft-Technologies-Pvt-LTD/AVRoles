...
const DoctorManagement = () => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-3xl font-extrabold text-gray-900 mb-8">Doctor Management</h1>
<div className="flex justify-end">
  <button
    onClick={() => setShowModal(true)}
    className="mb-6 inline-block bg-slate-900 hover:bg-slate-700 text-white font-semibold px-6 py-3 rounded-md shadow-md transition"
  >
    + Add Doctor
  </button>
</div>
            ...
            {doctors.length ? doctors.map((doc, i) => (
              <tr key={i} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <button onClick={() => setViewDoctor(doc)} className="text-indigo-600 hover:text-indigo-900 font-semibold underline">View Details</button>
                </td>
              </tr>
            )) : (
              <tr>
          ...
          <div className=" bg-white rounded-lg shadow-lg max-w-2xl w-full max-h-[85vh] overflow-y-auto p-4">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div className="col-span-2 flex justify-end space-x-3 mt-4">
                <button type="button" onClick={() => setShowModal(false)} className="bg-gray-300 hover:bg-gray-400 px-4 py-1.5 rounded text-sm">Cancel</button>
                <button type="submit" className="bg-slate-600 hover:bg-slate-700 text-white px-4 py-1.5 rounded text-sm">Submit</button>
              </div>
            </form>
          </div>
        </div>
